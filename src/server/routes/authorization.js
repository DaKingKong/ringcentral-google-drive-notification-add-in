...
async function openAuthPage(req, res) {
    try {
        ...
        const url = `${oauthApp.code.getUri()}&access_type=offline`;
...
async function getUserInfo(req, res) {
    ...
    const subscription = user.subscriptions.find(s => s.rcWebhookUri == rcWebhookUri);
    res.json({ user, hasSubscription: subscription != null });
}
